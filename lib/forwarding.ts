...
export async function getForwardingRules(): Promise<ForwardingRule[]> {
  const rows = await prisma.$queryRaw<Record<string, unknown>[]>`
    SELECT id, phone_sid, phone_number, forward_to, enabled FROM "tw_forwarding_rules"
  `
...
export async function upsertForwardingRule(input: {
  ...
  await prisma.$executeRaw`
    INSERT INTO "tw_forwarding_rules" (phone_sid, phone_number, forward_to, enabled, updated_at)
    VALUES (${input.phoneSid}, ${input.phoneNumber}, ${input.forwardTo}, ${input.enabled}, CURRENT_TIMESTAMP)
