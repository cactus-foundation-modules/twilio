    ...
    return NextResponse.json({
      numbers: numbers.map((n) => {
        ...
        return {
          ...
          forwardToSecond: rule?.forwardToSecond ?? '',
          ...
          forwardAttempts: rule?.forwardAttempts ?? 1,
          ...
          missedCallSmsEnabled: rule?.missedCallSmsEnabled ?? false,
          missedCallSmsMessage: rule?.missedCallSmsMessage ?? '',
          transcribeVoicemail: rule?.transcribeVoicemail ?? false,
          transcribeCalls: rule?.transcribeCalls ?? false,
          anonymousCallers: rule?.anonymousCallers ?? 'allow',
